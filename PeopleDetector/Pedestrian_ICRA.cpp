...
const int PeopleDetector::HUMAN_height(108);
const int PeopleDetector::HUMAN_width(36);
const int PeopleDetector::HUMAN_xdiv(9);
const int PeopleDetector::HUMAN_ydiv(4);
const int PeopleDetector::EXT(1);

// ---------------------------------------------------------------------
// Helper functions

PeopleDetector::PeopleDetector(void):scanner(HUMAN_height,HUMAN_width,HUMAN_xdiv,HUMAN_ydiv,256,0.8)
{
	// The detector
...
void PeopleDetector::PostProcess(std::vector<CRect>& result,const int combine_min)
{
...
void PeopleDetector::RemoveCoveredRectangles(std::vector<CRect>& result)
{
...
void PeopleDetector::LoadCascade()
{
	...
	scanner.LoadDetector(types,upper_bounds,filenames);
	// You can adjust these parameters for different speed, accuracy etc
	//scanner.cascade->nodes[0]->thresh += 0.8;
	//scanner.cascade->nodes[1]->thresh -= 0.295;
	scanner.cascade->nodes[0]->thresh += 0.1;
	scanner.cascade->nodes[1]->thresh -= 0.005;
}
...
void DetectionScanner::LoadDetector(std::vector<NodeDetector::NodeType>& types,std::vector<int>& upper_bounds,std::vector<std::string>& filenames)
{
	...
	for(unsigned int i=0;i<depth;i++)
		cascade->AddNode(types[i],(xdiv-PeopleDetector::EXT)*(ydiv-PeopleDetector::EXT)*baseflength,upper_bounds[i],filenames[i].c_str());
		
	hist.Create(1,baseflength*(xdiv-PeopleDetector::EXT)*(ydiv-PeopleDetector::EXT));
}
...
// initialization -- compute the Census Transform image for CENTRIST
...
void DetectionScanner::InitIntegralImages(const int stepsize)
{
	...
	for(int i=0;i<xdiv-PeopleDetector::EXT;i++)
	{
		const int xoffset = height/xdiv*i;
		for(int j=0;j<ydiv-PeopleDetector::EXT;j++)
		{
    ...
    while(image.nrow>=height && image.ncol>=width)
    {
        ...
        for(int i=2;i+height<image.nrow-2;i+=stepsize)
        {
            ...
            for(int j=2;j+width<image.ncol-2;j+=stepsize)
            {
				...
				int* p = hist.buf;
				hist.Zero();
				for(int k=0;k<xdiv-PeopleDetector::EXT;k++)
				{
					for(int t=0;t<ydiv-PeopleDetector::EXT;t++)
					{
						for(int x=i+k*hd+1;x<i+(k+1+PeopleDetector::EXT)*hd-1;x++)
						{
							int* ctp = ct.p[x];
							for(int y=j+t*wd+1;y<j+(t+1+PeopleDetector::EXT)*wd-1;y++)
								p[ctp[y]]++;
						}
						p += baseflength;
					}
...
int PeopleDetector::DetectHuman(const cv::Mat &image, std::vector<cv::Rect> &detection)
{
	std::vector<CRect> results;
	IntImage<double> original;
	cv::Mat img2Write;
	image.copyTo(img2Write);
	IplImage img = img2Write;
	original.Load(&img);//Load image
	...
	//copy result to std::vector
	detection.reserve(results.size());
	for (size_t i = 0;i<results.size();++i)
	{
		...
		detect.x = r.left;
		detect.y = r.top;
		detect.width = r.right-r.left;
		detect.height = r.bottom -r.top;
		detection.push_back(detect);
	}
